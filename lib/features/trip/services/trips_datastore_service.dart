...
class TripsDataStoreService {
  ...
  Stream<List<Trip>> listenToTrips() {
    return Amplify.DataStore.observeQuery(
      ...
      sortBy: [Trip.STARTDATE.ascending()],
        ...
        .map((event) => event.items.where((element) => element.endDate.getDateTime().isAfter(DateTime.now())).toList())
        .handleError(
      (error) {
  ...
  Stream<List<Trip>> listenToPastTrips() {
    return Amplify.DataStore.observeQuery(
      ...
      sortBy: [Trip.STARTDATE.ascending()],
        ...
        .map((event) => event.items.where((element) => element.endDate.getDateTime().isBefore(DateTime.now())).toList())
        .handleError(
      (error) {
        debugPrint('listenToTrips: A Stream error happened');
      },
  ...
  Stream<Trip> getTripStream(String id) {
    final tripStream = Amplify.DataStore.observeQuery(Trip.classType, where: Trip.ID.eq(id))
