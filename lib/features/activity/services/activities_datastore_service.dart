...
import 'package:bbb/models/Activity.dart';
import 'package:flutter/material.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

final activitiesDataStoreServiceProvider = Provider<ActivitiesDataStoreService>((ref) {
...
class ActivitiesDataStoreService {
  ...
  Stream<List<Activity>> listenToActivitiesForTrip(String tripId) {
    return Amplify.DataStore.observeQuery(
      Activity.classType,
      sortBy: [Activity.ACTIVITYDATE.ascending(), Activity.ACTIVITYTIME.ascending()],
    ).map((event) => event.items.where((element) => element.trip.id == tripId).toList()).handleError(
      (dynamic error) {
  ...
  Stream<Activity> listenToActivity(String id) {
    ...
    ).map((event) => event.items.where((element) => element.id == id).first).handleError(
      (dynamic error) {
